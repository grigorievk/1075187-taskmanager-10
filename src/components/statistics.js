...
import {isOneDay} from "../utils/date-time";
...
export default class Statistics extends AbstractSmartComponent {
  constructor({taskData, dateFrom, dateTo}) {
    super();
  ...
  show() {
    ...
    this.rerender(this._taskData, this._dateFrom, this._dateTo);
  }

  recoveryListeners() {}

  rerender(taskData, dateFrom, dateTo) {
    this._taskData = taskData;
    this._dateFrom = dateFrom;
    this._dateTo = dateTo;
  ...
  _renderCharts() {
    ...
    this._daysChart = renderDaysChart(daysCtx, this._taskData.getTasks(), this._dateFrom, this._dateTo);
    this._tagsChart = renderTagsChart(tagsCtx, this._taskData.getTasks());
    this._colorsChart = renderColorsChart(colorsCtx, this._taskData.getTasks());
  }
  ...
  _applyFlatpickr(element) {
    ...
    this._flatpickr = flatpickr(element, {
      ...
      onChange: (dates) => {
        if (dates.length === 2) {
          this.rerender(this._taskData, dates[0], dates[1]);
        }
      }
    });
  }
  ...
  getTemplate() {
    return createStatisticsTemplate({tasks: this._taskData.getTasks(), dateFrom: this._dateFrom, dateTo: this._dateTo});
  }
}
